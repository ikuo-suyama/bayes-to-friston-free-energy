...
ベイズ推論の文脈で、観測者が直接手にしているのはデータ $${{x}}$$ だけであり、世界の真の状態 $${{\theta}}$$ は観測できない。

生成モデル $${{p(x,\theta) = p(x\mid\theta)p(\theta)}}$$ のもとで、「$${{x}}$$ が観測される確率」を定義する方法は、全確率の法則を使って $${{\theta}}$$ を周辺化することである：

$$
p(x) = \int p(x\mid\theta)p(\theta)d\theta = \mathbb{E}_{p(\theta)}[p(x\mid\theta)]
$$

これは、「あり得るすべての世界（$${{\theta}}$$）の下での起こりやすさを、信念 $${{p(\theta)}}$$ で重み付けして平均する」という自然な帰結である。

この $${{p(x)}}$$ は**エビデンス（evidence）** または**周辺尤度（marginal likelihood）** と呼ばれ、**モデル全体としての予測分布**を表す。

（※ エビデンスが「周辺尤度」とも「正規化定数」とも呼ばれる理由については、付録2を参照）
...
## 付録2：p(x)の二つの見方：生成と推論

本文では、エビデンス $${{p(x)}}$$ を簡潔に「モデル全体の予測分布」として導入した。しかし、より深く理解するために重要な疑問がある。

**なぜ同じ積分 $${{p(x) = \int p(x|\theta)p(\theta)\mathrm{d}\theta}}$$ が、「周辺尤度」とも「正規化定数」とも呼ばれるのか？**

これは単なる用語の混乱ではない。同じ数式が、**何を固定して何を変数とみなすか**によって、まったく異なる役割を持つことを示している。

この付録では、生成と推論という二つの視点から $${{p(x)}}$$ を理解することで、この疑問に答える。

### 生成の視点：θを固定し、xをサンプリング

まず、Section 1 で見た生成過程を振り返ろう。

生成モデルでは、世界は次の順序で生成されると仮定されていた：
...
すなわち、
1. まず $${{\theta}}$$ が事前分布 $${{p(\theta)}}$$ からサンプリングされる
2. 次に、その $${{\theta}}$$ を条件として、$${{x}}$$ が $${{p(x|\theta)}}$$ からサンプリングされる
...
ここで重要な問いが生まれる。**「この生成モデル全体として、$${{x}}$$ が観測される確率はいくらか？」**

観測者の立場では、**どの $${{\theta}}$$ が真だったのかは分からない**。分かるのは「$${{x}}$$ が起きた」という事実だけである。

したがって、「この生成モデルのもとで $${{x}}$$ が観測される確率」を定義する唯一の方法は、全確率の法則を使うことだ。離散なら和、連続なら積分になる：
...
> 「どの世界（$${{\theta}}$$）にいるか分からない以上、あり得るすべての世界の下での起こりやすさを、信念 $${{p(\theta)}}$$ で重み付けして平均する」
...
$$
p(x) = \mathbb{E}_{p(\theta)}[p(x|\theta)]
$$

であり、$${{p(x)}}$$ は「$${{\theta}}$$ について周辺化（marginalization）した量」である。

**この文脈では、$${{p(x)}}$$ は「モデル全体の予測分布」であり、エビデンス（evidence）または周辺尤度（marginal likelihood）と呼ばれる。**
