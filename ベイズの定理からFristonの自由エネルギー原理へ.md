...
ここで、本記事で頻出する用語を整理しておく。

**事後分布 $${{p(\theta\mid x)}}$$：**
- 観測 $${{x}}$$ を得た後の $${{θ}}$$ に関する信念
- ベイズの定理: $${{p(\theta\mid x) = \frac{p(x\mid\theta)p(\theta)}{p(x)}}}$$ で計算される

**同時分布 $${{p(x,\theta)}}$$：**
- 生成モデル全体を表す分布
- **尤度×事前分布**: $${{p(x,\theta) = p(x\mid\theta)p(\theta)}}$$

**尤度 $${{p(x\mid\theta)}}$$：**
- $${{θ}}$$ を与えたとき、$${{x}}$$ が観測される条件付き確率
- 推論の文脈では「$${{θ}}$$ のもっともらしさを評価する関数」として使われる

**事前分布 $${{p(\theta)}}$$：**
- 観測前に持っていた $${{θ}}$$ に関する信念
- $${{θ}}$$ がどの値をとりやすいかの事前知識を表す

**エビデンス $${{p(x)}}$$：**
- 周辺尤度、または予測分布とも呼ばれる
- $${{p(x) = \int p(x\mid\theta)p(\theta)d\theta}}$$ として、$${{θ}}$$ について周辺化した量
- モデル全体としての「$${{x}}$$ の起こりやすさ」を表す
...
この量は**エビデンス（evidence）** または**周辺尤度（marginal likelihood）** と呼ばれ、次節で見る「尤度」との対比において重要な役割を果たす。

興味深いことに、この同じ積分は、ベイズの定理

$$
p(\theta|x) = \frac{p(x|\theta)p(\theta)}{p(x)}
$$

において**正規化項**としても機能する。つまり、事後分布 $${{p(\theta|x)}}$$ を「$${{\theta}}$$ についての確率分布」にするために分母に必要な量でもある。

同じ式 $${{p(x) = \int p(x|\theta)p(\theta)\mathrm{d}\theta}}$$ が「予測分布」と「正規化項」という二つの役割を持つ理由については、付録2で詳しく説明する。

### エビデンスと尤度：log変換前の対比

ここで、2つの重要な量を比較しよう。どちらもlogを取る前の段階で考える。

**尤度 $${{p(x\mid\theta)}}$$：**
- 「特定の $${{θ}}$$ が真だと仮定したとき、$${{x}}$$ が観測される確率」
- $${{θ}}$$ を固定、$${{x}}$$ を変数として見る（生成の立場）
- ただし推論では「$${{x}}$$ を固定して $${{θ}}$$ を評価する関数」として使う

**エビデンス $${{p(x)}}$$：**
- 「$${{θ}}$$ について不確実性を残したまま、$${{x}}$$ が観測される確率」
- $${{p(x) = \int p(x\mid\theta)p(\theta)d\theta}}$$ として全 $${{θ}}$$ を周辺化
- モデル全体としての予測分布

つまり、
- 尤度：「ある1つの世界（$${{θ}}$$）の下での予測」
- エビデンス：「あり得るすべての世界を平均した予測」

この違いが、次に見る「驚き」の定義に直結する。
...
logを取ると、尤度とエビデンスは次のように変換される：

**対数尤度 $${{  \log p(x\mid\theta)}}$$：**
- 「特定の $${{θ}}$$ の下で、$${{x}}$$ がどれくらい説明されるか」の評価量
- 最大化すると最尤推定（MLE）になる

**驚き $${{-\log p(x)}}$$：**
- 「モデル全体から見て、$${{x}}$$ がどれくらい想定外か」の評価量
- 小さいほど「予測できていた」ことを意味する
...
驚きとは、特定の仮説 $${{θ}}$$ に対する違和感ではなく、**自分が信じていた"世界全体"（エビデンス）に対する想定外度**である。

これがFristonの自由エネルギー原理で「驚きを最小化する」という表現が単なる比喩ではなく、数学的に定義された量であることを意味する。
...
ここで重要なのは、この式は**計算可能**である点だ：

- $${{p(x,\theta) = p(x\mid\theta)p(\theta)}}$$：事前分布と尤度から計算できる
- $${{q(\theta)}}$$：自分で仮定した分布なので既知
- 両項とも $${{q}}$$ についての期待値なので、積分（またはサンプリング）で評価できる

つまり、**計算困難だった $${{-\log p(x)}}$$ の代わりに、計算可能な上界 $${{\mathcal{F}(q)}}$$ を手に入れた**ことになる。
...
ここで決定的に重要なのは、**$${{-\log p(x)}}$$ は $${{q}}$$ に依存しない定数**であることだ。

観測 $${{x}}$$ が与えられた後、$${{-\log p(x)}}$$ は固定された値であり、$${{q}}$$ をどう選んでも変わらない。

したがって、$${{q}}$$ について最適化する際、この項は消える：

$$
\arg\min_q \mathcal{F}(q) = \arg\min_q \left[ -\log p(x) + \mathrm{KL}\left( q\|p(\theta|x) \right) \right] = \arg\min_q \mathrm{KL}\left( q\|p(\theta|x) \right)
$$

これが意味するのは：

> **$${{\mathcal{F}(q)}}$$ を最小化することと、$${{\mathrm{KL}\left( q\|p(\theta|x) \right)}}$$ を最小化することは完全に同値である**

KLは非負で、0になるのは $${{q = p(\theta|x)}}$$ のときだけ。

したがって、**$${{\mathcal{F}(q)}}$$ の最小化は、計算できない真の事後分布 $${{p(\theta|x)}}$$ に、計算可能な $${{q(\theta)}}$$ を近づけることを数学的に保証している**。

これが変分自由エネルギーの核心であり、「計算できるものだけで目的関数を作ったら、それを最適化することが計算できない分布に近づくことを保証していた」というトリックである。
...
$$
\mathcal{F}(q) = -\log p(x) + \mathrm{KL}\left( q\|p(\theta|x) \right)
$$
...
- $${{-\log p(x)}}$$：世界が本当にどれくらい驚くべきだったか（真の驚き）
- $${{\mathrm{KL}\left( q\|p(\theta|x) \right)}}$$：自分の信念 $${{q(\theta)}}$$ が、真の事後分布からどれだけズレているか（認識のズレ）
...
自由エネルギーは驚きの上界であり、その差がKL（近似誤差）である。
...
つまり、**「自分がどれだけ驚くべきだったか」を正しく理解する方向に信念が更新される**。
...
この最小化問題を、正規化条件 $$\int q(s)\mathrm{d}s = 1$$ の制約のもとでラグランジュ未定乗数法により解くと、解として**ボルツマン分布**が得られる：
...
## 付録2：周辺尤度が正規化定数になる必然性

本文では、エビデンス $${{p(x)}}$$ を「モデル全体の予測分布」として導入し、ベイズの定理において「正規化項」としても機能することを簡単に説明した。

ここでは、より深く理解するために重要な疑問に答える。

**なぜ同じ積分 $${{p(x) = \int p(x|\theta)p(\theta)\mathrm{d}\theta}}$$ が、「予測分布（周辺尤度）」と「正規化項（正規化定数）」という二つの役割を持つのか？**

これは単なる用語の混乱ではない。同じ数式が、**何を固定して何を変数とみなすか**によって、まったく異なる役割を持つことを示している。

この付録では、生成と推論という二つの視点から $${{p(x)}}$$ を理解することで、この疑問に答える。

### 生成の視点：θを周辺化してxの分布を作る

生成モデル $${{p(x,\theta) = p(x|\theta)p(\theta)}}$$ において、**「この世界で $${{x}}$$ が観測される確率は？」** という問いに答えるには、$${{\theta}}$$ を周辺化する：

$$
p(x) = \int p(x|\theta)p(\theta)\mathrm{d}\theta = \mathbb{E}_{p(\theta)}[p(x|\theta)]
$$

この操作で何が起きているか：
- **$${{\theta}}$$ の扱い**：サンプリング → 全 $${{\theta}}$$ について平均化（周辺化）
- **作られる分布**：$${{x}}$$ の分布（予測分布）
- **$${{p(x)}}$$ の意味**：モデル全体として「$${{x}}$$ がどれくらい起こりやすいか」

つまり、**$${{\theta}}$$ を積分で消すことで、$${{x}}$$ についての分布を作る操作**である。

### 推論の視点：xを固定し、θを推論

次に、視点を変えよう。今度は $${{x}}$$ が**観測された後**の話である。

ベイズ推論の目的は、観測 $${{x}}$$ を得たあとで、「真の世界の状態 $${{\theta}}$$ がどの値をとっている可能性が高いか」を推定することである。

これは事後分布 $${{p(\theta|x)}}$$ として定式化される。ベイズの定理により：

$$
p(\theta|x) = \frac{p(x|\theta)p(\theta)}{p(x)}
$$

ここで重要な点は、**$${{p(\theta|x)}}$$ は $${{\theta}}$$ についての確率分布である**ということだ。

確率分布である以上、必ず

$$
\int p(\theta|x)\mathrm{d}\theta = 1
$$
...
$$
\int p(\theta|x)\mathrm{d}\theta = \int \frac{p(x|\theta)p(\theta)}{p(x)}\mathrm{d}\theta = \frac{1}{p(x)}\int p(x|\theta)p(\theta)\mathrm{d}\theta
$$

これが 1 になるためには、分母 $${{p(x)}}$$ が次のように決まっていなければならない：

$$
p(x) = \int p(x|\theta)p(\theta)\mathrm{d}\theta
$$

つまり、$${{p(x)}}$$ は**事後分布を「$${{\theta}}$$ についての確率分布」にするために必要な正規化定数（normalizing constant）** として、必然的に現れる。

**この文脈では、$${{p(x)}}$$ は「$${{\theta}}$$ を正規化（normalization）するための量」であり、推論の目的そのものではなく、計算上の副産物である。**
...
が、文脈によってまったく異なる意味を持つことを見てきた。

**2つの視点の対比：**

**【生成の視点】**
- **何を作る分布か**：$${{x}}$$ の分布（予測分布）
- **$${{  \theta}}$$ の扱い**：サンプリング → 周辺化（平均）
- **$${{  p(x)}}$$ の役割**：周辺尤度/予測分布
- **意味**：モデル全体として $${{x}}$$ がどれくらい起こりやすいか

**【推論の視点】**
- **何を作る分布か**：$${{\theta}}$$ の分布（事後分布）
- **$${{  x}}$$ の扱い**：観測済みで固定
- **$${{  p(x)}}$$ の役割**：正規化定数
- **意味**：$${{\theta}}$$ を確率分布にするために必要な定数

**同じ積分 $${{  \int p(x|\theta)p(\theta)\mathrm{d}\theta}}$$ が、何についての確率分布を作るかで役割が変わる：**

- **生成の文脈**：$${{\theta}}$$ について平均化 → $${{x}}$$ の分布を作る → 予測分布
- **推論の文脈**：$${{\theta}}$$ について正規化 → $${{\theta}}$$ の分布を作る → 正規化定数

「周辺化」と「正規化」は同じ操作を、見る向き（$${{x}}$$ 側か $${{\theta}}$$ 側か）によって呼び分けているだけである。

私たちは普段、「まず尤度×事前を書く → あとで割って正規化する」という順で考える。このため、「たまたま同じ積分が正規化定数にもなっている」ように見える。

しかし本質は逆である。**生成モデル $${{p(x,\theta)}}$$ は最初から正規化されている**（$${{  \int\int p(x,\theta)\mathrm{d}x\mathrm{d}\theta = 1}}$$）。

- $${{\theta}}$$ を周辺化すれば $${{p(x)}}$$
- $${{x}}$$ を周辺化すれば $${{p(\theta)}}$$

どちらも元の正規化から自動的に従う。**見る向きによって、同じ量の意味が変わるだけである。**
