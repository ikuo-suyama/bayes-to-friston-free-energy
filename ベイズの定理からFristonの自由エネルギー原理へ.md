...
### 2.2 p(x)の二つの見方：生成と推論

ここまでで、驚き $${{-\log p(x)}}$$ が計算困難であることを見てきた。しかし、その前に立ち止まって考えるべき重要な疑問がある。
...
この二つの見方を理解することは、変分推論で $${{p(x)}}$$ がなぜ「計算できない問題」になるのかを正しく理解する鍵となる。

#### 2.2.1. 生成の視点：θを固定し、xをサンプリング
...
| 視点 | 固定されているもの | 変数 | $${{p(x)}}$$ の役割 | 文脈 |
|------|------------------|------|-----------|------|
| **生成** | なし（$${{\theta}}$$ をサンプリング→周辺化） | $${{x}}$$ | 周辺尤度/予測分布 | モデル全体の予測 |
| **推論** | $${{x}}$$（観測データ） | $${{\theta}}$$ | 正規化定数 | 事後分布の構築 |
...
**生成モデル $${{p(x,\theta)}}$$ は最初から正規化されている**（$${{  \int\int p(x,\theta)\mathrm{d}x\mathrm{d}\theta = 1}}$$）。
...
**見る向きによって、同じ量の意味が変わるだけである。**

#### 二つの視点を踏まえて

この二つの視点を理解した上で、次に log 変換がもたらす対比を見ていく。

特に重要なのは、「驚き $${{-\log p(x)}}$$」と「対数尤度 $${{  \log p(x|\theta)}}$$」の違いである。

- $${{p(x)}}$$：「モデル全体の予測」（生成の視点）
- $${{p(x|\theta)}}$$：「特定の $${{\theta}}$$ の下での尤もらしさ」（推論で評価する関数）

log を取っても、この根本的な違いは変わらない。

次節では、この違いが log 変換後にどのような対比として現れるのかを見ていく。

### 2.3 log変換後の対比：対数尤度と驚き
